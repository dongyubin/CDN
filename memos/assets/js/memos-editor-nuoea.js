...
function getEditIcon() {
  ...
  async function uploadImage(data) {
    ...
    let blobUrl = `${memosPath}/api/v1/resource/blob?openId=${memosOpenId}`;
  ...
  async function updateAvatarUrl(e) {
    try {
      let response = await fetch(`${memosPath}/api/v1/user/me?openId=${memosOpenId}`);
      if (response.ok) {
        let resdata = await response.json();
        e.forEach(item => {
